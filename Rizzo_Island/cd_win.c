...
void CDAudio_SysEject(void)
{
	...
	if ((dwReturn = mciSendCommand(wDeviceID, MCI_SET, MCI_SET_DOOR_OPEN, (DWORD_PTR)NULL)))
		Con_DPrintf("MCI_SET_DOOR_OPEN failed (%x)\n", (unsigned)dwReturn);
}
...
void CDAudio_SysCloseDoor(void)
{
	...
	if ((dwReturn = mciSendCommand(wDeviceID, MCI_SET, MCI_SET_DOOR_CLOSED, (DWORD_PTR)NULL)))
		Con_DPrintf("MCI_SET_DOOR_CLOSED failed (%x)\n", (unsigned)dwReturn);
}
...
int CDAudio_SysPlay (int track)
{
	...
	if (dwReturn)
	{
		Con_DPrintf("CDAudio_SysPlay: MCI_STATUS failed (%x)\n", (unsigned)dwReturn);
		return -1;
	}
	if (mciStatusParms.dwReturn != MCI_CDA_TRACK_AUDIO)
	{
		Con_DPrintf("CDAudio_SysPlay: track %i is not audio\n", track);
		return -1;
	}
	...
	if (dwReturn)
	{
		Con_DPrintf("CDAudio_SysPlay: MCI_PLAY failed (%x)\n", (unsigned)dwReturn);
		return -1;
	}
...
int CDAudio_SysPause (void)
{
	...
	{
		Con_DPrintf("MCI_PAUSE failed (%x)\n", (unsigned)dwReturn);
		return -1;
	}
	return 0;
}
...
int CDAudio_SysResume (void)
{
	...
	if (dwReturn)
	{
		Con_DPrintf("CDAudio_SysResume: MCI_PLAY failed (%x)\n", (unsigned)dwReturn);
		return -1;
	}
	return 0;
}
...
int CDAudio_SysStartup (void)
{
	...
	{
		Con_DPrintf("CDAudio_SysStartup: MCI_OPEN failed (%x)\n", (unsigned)dwReturn);
		return -1;
	}
	...
	{
		Con_DPrintf("CDAudio_SysStartup: MCI_SET_TIME_FORMAT failed (%x)\n", (unsigned)dwReturn);
